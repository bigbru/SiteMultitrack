        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (action != null) {
                switch (action) {
                    ...
                    case "playSong": {
                        currentPagination = 0;
                        Chanson c = gestionnaireSongs.getSongById(Long.parseLong(request.getParameter("id")));
                        out.print("[{\"id\":\"" + c.getId() + "\", \"titre\":\"" + c.getTitre()+ "\"}]");
                        break;
                    }
                    
                    case "getASong": {
                        Collection<Chanson> c = new LinkedList<>();
                        c.add(gestionnaireSongs.getSongById(Long.parseLong(request.getParameter("id"))));
                        out.print(gestionnaireSongs.listSongsToJson(c));
                        break;
                    }
